...
// Latitude→temperature: 0.87 at equator (+27°C), 0.35 at pole (-25°C).
// Calibrated against NASA GISS zonal annual-mean climatology.
// Old curve (1 - lat^2*1.15 - lat^3*0.1) overshot at equator (+40°C)
// and undershot at poles (-60°C). See tools/probe_temperature.mjs.
temperature[i]=Math.max(0,Math.min(1,0.87-Math.pow(lat,1.5)*0.42-lat*lat*lat*0.10-Math.max(0,elevation[i])*.4+fbm(nx*3+80,ny*3+80,3,2,.5)*.08));}
...
tGrid[my*mW2+mx]=Math.max(0,Math.min(1,0.87-Math.pow(lt,1.5)*0.42-lt*lt*lt*0.10+Math.exp(-((lt-0.20)*(lt-0.20))/(2*0.08*0.08))*0.06-Math.max(0,e2)*0.45));}
...
const locT=Math.max(0,Math.min(1,0.87-Math.pow(lt,1.5)*0.42-lt*lt*lt*0.10+Math.exp(-((lt-0.20)*(lt-0.20))/(2*0.08*0.08))*0.06-Math.max(0,e2)*0.45));
...
const bt=0.87-Math.pow(tLat,1.5)*0.42-tLat*tLat*tLat*0.10+shE-Math.max(0,e)*0.45+fbm(nx*3+80,ny*3+80,3,2,.5)*.08+fbm(nx*1.2+55,ny*1.2+55,3,2,.55)*.10;
...
moisture[i]=Math.max(.02,Math.min(1,m));
temperature[i]=Math.max(0,Math.min(1,0.87-Math.pow(lat,1.5)*0.42-lat*lat*lat*0.10-Math.max(0,e)*.4+fbm(nx*3+80,ny*3+80,3,2,.5)*.1));}
...
elevation[i]=e;temperature[i]=Math.max(0,Math.min(1,0.87-Math.pow(lat,1.5)*0.42-lat*lat*lat*0.10-Math.max(0,e)*.4+fbm(nx*3+80,ny*3+80,3,2,.5)*.1));}
