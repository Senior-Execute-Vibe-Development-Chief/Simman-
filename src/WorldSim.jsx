...
temperature[i]=Math.max(0,Math.min(1,1-Math.pow(lat,2.0)*1.1-lat*lat*lat*0.1-Math.max(0,elevation[i])*.4+fbm(nx*3+80,ny*3+80,3,2,.5)*.08));}
...
tGrid[my*mW2+mx]=Math.max(0,Math.min(1,1-Math.pow(lt,2.0)*1.1-lt*lt*lt*0.1+Math.exp(-((lt-0.20)*(lt-0.20))/(2*0.08*0.08))*0.06-Math.max(0,e2)*0.55));}
...
// IRL temperature: red extends to ~30° lat, yellow to ~45°, blue at ~60°, purple at ~75°+
// Higher exponent (2.0) keeps temp warm longer, then drops steeply at high latitudes
const bt=1-Math.pow(tLat,2.0)*1.1-tLat*tLat*tLat*0.1+shE-Math.max(0,e)*0.55+fbm(nx*3+80,ny*3+80,3,2,.5)*.08+fbm(nx*1.2+55,ny*1.2+55,3,2,.55)*.10;
...
moisture[i]=Math.max(.02,Math.min(1,m));
temperature[i]=Math.max(0,Math.min(1,1-Math.pow(lat,2.0)*1.1-lat*lat*lat*0.1-Math.max(0,e)*.4+fbm(nx*3+80,ny*3+80,3,2,.5)*.1));}
...
elevation[i]=e;temperature[i]=Math.max(0,Math.min(1,1-Math.pow(lat,2.0)*1.1-lat*lat*lat*0.1-Math.max(0,e)*.4+fbm(nx*3+80,ny*3+80,3,2,.5)*.1));}
