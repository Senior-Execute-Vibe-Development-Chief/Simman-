...
export const TUNING_SCHEMA = [
  ...
  },
  {
    ...
    params: [
      ...
      { key: "WAR_DEPTH", label: "Strategic depth", def: 0.25, min: 0, max: 1.0, step: 0.05,
        desc: "An ESTABLISHED realm (large total military) funnels reserves to a threatened frontier, defending harder than the lone border garrison — defence x (1 + this x log(total-might) / (1 + fronts)). So a big old power isn't beaten at a point of contact by a small upstart with one strong army; but that reserve is split across fronts, so a realm pressed on all sides defends each weakly. 0 = off (no depth; frontier defends with only its local garrison, so small new powers overrun big ones)." },
