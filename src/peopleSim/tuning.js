...
export const TUNING_SCHEMA = [
  ...
  },
  {
    ...
    params: [
      ...
      { key: "TIER_SCALE_REF", label: "Tier-scale reference pop", def: 40000, min: 1000, max: 200000, step: 1000,
        desc: "RELATIVE TIERS: the population bar to count as a town/city/metropolis scales with the WORLD's total population ÷ this reference (capped by TIER_SCALE_MAX). As civilisation grows, the size that qualifies as a 'city' rises too, so the urban hierarchy stays proportional and the rural majority stays rural — instead of mid-size farming villages being mislabelled 'towns' once the world fills up. Lower = the bar rises faster (fewer, larger towns/cities). Set huge to disable (fixed absolute thresholds, the old behaviour)." },
      { key: "TIER_SCALE_MAX", label: "Tier-scale cap", def: 2.0, min: 1, max: 6, step: 0.25,
        desc: "Ceiling on the relative-tier multiplier (above), so the bar can't rise so far that genuine cities vanish. At 2.0 a 'town' needs ~2× the base 250 (≈500) and a 'city' ~2× 1200 (≈2400) once the world is large — reclassifying the smallest 'towns' back to rural farming regions while keeping real cities. Up = a harsher cut (more rural, fewer urban centres)." },
