...
export function mirrorIdentityField(world) {
  ...
  for (let ti = 0; ti < N; ti++) {
    ...
    const base = ti * K;
    if (!skipCul) writeMix(culId, culShr, base, s.culMix);
...
export function tileCulShareOf(world, ti, culId) {
  const idA = world.tileCulId; if (!idA || culId < 0) return 0;
  const base = ti * IDENTITY_K, shA = world.tileCulShr;
  for (let k = 0; k < IDENTITY_K; k++) {
    const id = idA[base + k]; if (id < 0) break;
    if (id === culId) return shA[base + k] / 255;
  }
  return 0;
}
