...
export const TUNING_SCHEMA = [
  ...
  },
  {
    ...
    params: [
      { key: "LEARN_BASE", label: "Tech learning speed", def: 0.000006, min: 0.000003, max: 0.0002, step: 0.0000005,
        desc: "Master scaling on all knowledge growth. Calibrated (with ORG_LIT_BRANCH, SCI_SPREAD and ORG_APT_LEARN) so the era ladder tracks the displayed calendar — the leading civilisation reaches the Bronze era around 2900 BC, Classical antiquity around 100 BC, Medieval around 400 AD, Industrial around 1600 and the Modern frontier near 1950 — rather than racing to industry by the classical period (which produced anachronistic multi-million cities in antiquity, and a leader at the modern era by 1000 AD). Up = the whole bronze→industrial arc plays out faster. On a large map the leading edge tracks the calendar but the periphery lags it — raise DIFFUSE_RATE (not this) to pull the hinterland up without making the leaders anachronistic. NOTE: the pace COMPOUNDS (a developed, winter-aptitude, competitive leader learns far faster than a fresh cradle), so this base alone can't flatten the curve — the three companion knobs above brake the mid/late-game runaway." },
