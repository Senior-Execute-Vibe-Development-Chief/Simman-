...
const DSP = { glottalRefl: 0.9, damp: 0.997, radiation: 0.9, wallLoss: 1.3, wallThresh: 0.03, hf: 0.92, turb: 4 };
// `turb` = frication/burst injection gain. The bursts were tuned when the
// waveguide self-oscillated and amplified them; now that it's passive they need
// real gain, or stops/fricatives are inaudible ("sounds like only vowels").
...
function makeTract() {
  ...
  function shape(p) {
    ...
    for (let i = 0; i < N; i++) {
      ...
      wall[i] = Math.max(0, DSP.damp - DSP.wallLoss * clamp((DSP.wallThresh - minA) / DSP.wallThresh, 0, 1));   // a loss must stay in [0,1]; going negative inverted the wave at closures
    }
  ...
  function step(glottal, turb, lambda, noise, p, lipOut) {
    ...
    const inject = (index, gain, dia) => {
      ...
      const nz = turb * gain * thin * open * DSP.turb;
    ...
    jL[N - 1] = R[N - 1] * LIP_REFLECTION;   // lip: left-going wave INTO the last section (jL[m] = wave into section m)
    for (let i = 1; i < N; i++) {
    ...
    const HF = DSP.hf;
    for (let k = 0; k < N; k++) {
      let r = jR[k] * wall[k], l = jL[k] * wall[k];   // L[k] = jL[k] (was jL[k+1] — off by one → non-passive, self-oscillating)
    ...
    noseJL[NOSE_LENGTH - 1] = noseR[NOSE_LENGTH - 1] * LIP_REFLECTION;
    ...
    for (let k = 0; k < NOSE_LENGTH; k++) {
      const r = noseJR[k] * DSP.damp, l = noseJL[k] * DSP.damp;
...
// Tongue-BODY target per place (the formant "locus"). Leaning the tongue toward
// it during a consonant bends the neighbouring vowel's formants — that transition
// is the main perceptual cue for consonant PLACE. Without it, every stop was just
// a gap in an unchanging vowel-formant field ("sounds like only vowels"). Labials
// pull F2 low (tongue back), coronals/palatals high (front), velars mid; glottals
// (7) don't move the tongue body.
const LOCUS = { 0: 15, 8: 30, 1: 31, 2: 27, 3: 32, 4: 22, 5: 13, 6: 10 };
...
function scoreCons(B, c, t, kpts, final) {
  ...
  const trans = 0.028;
  // lean the tongue BODY toward this place's formant locus, so the flanking
  // vowels' formants transition into/out of the consonant — the place cue that
  // makes a stop sound like a consonant and not a gap. Glides (m===6) set their
  // own tongue posture below; glottals have no locus.
  const locus = LOCUS[p];
  if (locus != null && c.m !== 6) B.to("tongueIndex", t, locus, trans);
