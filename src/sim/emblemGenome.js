...
function tinctureOn(grounds, hue, val, poles = [], opts = {}) {
  ...
  if (uniform) {
    ...
    if (classOf(grounds[0]) === "metal") return T(namedDyeDark(hue, val, null, opts.bunting));
    // dark ground → a LIGHT mark. On cloth it is UNDYED cloth (argent, the
    // mill's default — the reason white dominates real flags and their metal
    // bands): gold thread only when the intent is GENUINELY gold (a weld-hue
    // choice), never merely because the hue runs warm — a red or orange intent
    // still bands white, not gold. Painted heraldry keeps the gilt-when-warm.
    if (opts.bunting) return T(hueFamily(dyeHue(hue)).includes("or") ? "or" : "argent");
    return T(nearestTincture(hsl(dyeHue(hue), msat, 0.62 + val * 0.3), METALS));
  }
  let best = null, bd = -1;
