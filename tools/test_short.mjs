...
for(let step=0;step<=300;step++){
  ...
  if(step%50===0){
    let kc=0,ports=0,alive=0;
    for(let i=0;i<cur.tribeSizes.length;i++){if(cur.tribeSizes[i]<=0)continue;alive++;
      kc+=cur.tribeKnownCoasts[i]?cur.tribeKnownCoasts[i].length:0;
      ports+=cur.tribePorts[i]?cur.tribePorts[i].length:0;}
    console.log(`Step ${step} (${yearStr(step)}): ${alive} tribes, ${kc} known coasts, ${ports} ports`);
  }
}
