...
async function curlRetry(url, extra = [], label = url, { tries = 5, wait0 = 60000 } = {}) {
  for (let i = 0, wait = wait0; ; i++, wait *= 2) {
    try {
...
// A file that exhausts its retries is SKIPPED, never fatal: the pass banks
...
for (const p of picks) {
  ...
  try {
    for (let att = 0; ; att++) {
      ...
      if (att >= 2) throw new Error("downloads keep coming back as non-media");
      await sleep(30000);
    }
    p.ok = true;
    fetched++;
...
console.log(`downloaded ${fetched}, kept ${skipped} already valid on disk${failed.length ? `; SKIPPED ${failed.length}: ${failed.join(" ")} — re-run to fill` : ""}`);
const banked = picks.filter(p => p.ok);

// SOURCES.tsv + manifest — writes ONLY what is verified on disk, so a pass
// with skips still leaves a coherent (if smaller) bank behind
const tsv = ["symbol\tfile\tcommons title\tlicence\tauthor\tsource"]
  .concat(banked.map(p => [p.sym, p.slug, p.title, p.licence, p.author, p.desc].join("\t")))
  .join("\n") + "\n";
writeFileSync(join(OUT_DIR, "SOURCES.tsv"), tsv);

// the manifest module the Lab imports (tiny — files stay in assets/)
const total = banked.reduce((a, p) => a + statSync(join(OUT_DIR, p.slug)).size, 0);
const manifest = `// GENERATED by tools/build_ipa_audio.mjs — do not edit by hand.
// ${banked.length} recorded phones (${(total / 1024) | 0} KB of audio in assets/ipa-audio/),
// covering ${banked.length}/${phones.length} of the generator's base phone space.
...
export const IPA_CLIPS = ${JSON.stringify(Object.fromEntries(banked.map(p => [p.sym, p.slug])), null, 0)};
`;
writeFileSync(MANIFEST, manifest);
console.log(`wrote ${MANIFEST.replace(ROOT + "/", "")} (${banked.length} clips), SOURCES.tsv, ${(total / 1024) | 0} KB of audio`);
if (failed.length) process.exitCode = 3;            // visible "gaps remain" signal for a supervisor
