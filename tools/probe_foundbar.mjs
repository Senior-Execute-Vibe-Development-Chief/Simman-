...
ensureYieldCv(world);
const deepMap = world._yieldDeep;
const lonOf = (x) => (x / TW) * 360 - 180, latOf = (y) => 90 - (y / TH) * 180;
// the REAL law's margin (crystallize.js leanAt): 1 / the per-component
// deep-year map, fert-weighted in harvest.js — this probe scores the code,
// not a local re-derivation (the one-copy discipline). The "POINT" column
// history (pre-fix composite-cv margins) lives in the 2026-08-25 run logs.
const marginAt = (i) => 1 / Math.max(0.2, deepMap[i]);
...
console.log(`  region        crop   margin p10/p50/p90   @5×clamp%   topfert-decile p50`);
for (const r of R) {
  const mp = [], top = [];
  for (let ty = 0; ty < TH; ty++) for (let tx = 0; tx < TW; tx++) {
    ...
    mp.push(marginAt(i));
    top.push([world.fert[i], marginAt(i)]);
  }
  ...
  console.log(`  ${r.k.padEnd(12)} ${String(mp.length).padStart(5)}  ${q(mp, .1).toFixed(2)}/${q(mp, .5).toFixed(2)}/${q(mp, .9).toFixed(2)}        ${String(clampP).padStart(3)}%       ${q(td.map(t => t[1]), .5).toFixed(2)}`);
}
console.log(`\n  READ: a founding lane can only mint where the margin is payable (~≤3×);`);
console.log(`  the peer capacity of a cell ≈ its census mass ÷ (bar × the margin at the`);
console.log(`  candidate tiles) — a region whose cropland reads mostly ≥4× seats ~nothing.`);
