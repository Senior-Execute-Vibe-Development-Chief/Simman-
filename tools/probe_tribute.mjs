...
console.log(`step | landNations realms | tribute: land / realm | treasury Σ | famine deaths Σ`);
for (let i = 0; i < STEPS; i++) {
  ...
  let tribLand = 0, tribRealm = 0, treas = 0;
  ...
  console.log(`${String(world.step).padStart(5)} | ${String(nLand).padStart(11)} ${String(nRealm).padStart(6)} | ${tribLand.toFixed(0).padStart(7)} / ${tribRealm.toFixed(0).padStart(6)} | ${treas.toFixed(0).padStart(9)} | ${starved}`);
}
