...
for (let done = 0; done < STEPS; done += 3000) {
  ...
  const eras = [], pStaple = [], pMetal = [], pMat = [];
  const sci = [], nAgri = [], nMet = [], nSea = [], fleets = [], hegs = [];
  const metRows = [];
  let n = 0;
  for (const s of world.settlements) {
    ...
    if (s._dbgNeedSea !== undefined) nSea.push(s._dbgNeedSea);
    if (s._dbgFleet !== undefined) fleets.push(s._dbgFleet);
    hegs.push(s._hegF || 0);
  ...
  const eMax = eras.length ? Math.max(...eras) : 0;
  console.log(`step ${String(world.step).padStart(6)}  n ${n} · era p90/max ${ERAS[q(eras, .9)] || "-"} / ${ERAS[eMax] || "-"}`);
  ...
  console.log(`   throttles p50/p90  sciMul ${q(sci, .5).toFixed(2)}/${q(sci, .9).toFixed(2)} · needAgri ${q(nAgri, .5).toFixed(2)}/${q(nAgri, .9).toFixed(2)} · needMetal ${q(nMet, .5).toFixed(2)}/${q(nMet, .9).toFixed(2)} · needSea ${q(nSea, .5).toFixed(2)}/${q(nSea, .9).toFixed(2)} · fleet ${q(fleets, .5).toFixed(2)} · hegF ${q(hegs, .5).toFixed(2)}/${q(hegs, .9).toFixed(2)}`);
