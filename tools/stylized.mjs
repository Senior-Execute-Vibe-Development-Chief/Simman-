...
for (const k of ["CROSS_REALM_HEIRS", "CLAIMANT_WARS", "CLAIM_POWER_WIN", "ADOPT_ADMIN", "ADOPT_ADMIN_DELAY", "CREDIT_RATE", "CREDIT_MAX_MULT", "CAP_MODEL", "CAP_FISC", "CAP_LOG"]) if (process.env[k] != null) { T[k] = +process.env[k]; console.log(`[stylized]   lever ${k}=${T[k]}`); }
