...
for(const s of world.settlements){if(s&&s.mode==="settled"&&s.countryId>=0&&(s.tier|0)>=2){let a=cityList.get(s.countryId);if(!a)cityList.set(s.countryId,a=[]);a.push(s);}}
const prov=new Int32Array(claim.length).fill(-1);
for(let ti=0;ti<claim.length;ti++){const cc=claim[ti];if(cc<0)continue;const arr=cityList.get(cc);if(!arr)continue;
  if(arr.length===1){prov[ti]=arr[0].id;continue;}
  const py=(ti/tw)+0.5,px=(ti-((ti/tw)|0)*tw)+0.5;let best=-1,bd=Infinity;
...
for(let ti=0;ti<claim.length;ti++){const py=(ti/tw)|0,px=ti-py*tw;const cc=claim[ti];let col;
  if(elev[ti]<=0)col=[18,32,64];else if(cc<0)col=[150,140,120];else{col=colByC.get(cc);if(!col){col=hsl(((cc*61)%360+360)%360,0.55,0.5);colByC.set(cc,col);}}
...
for(let ti=0;ti<claim.length;ti++){const cc=claim[ti];if(cc<0)continue;const py=(ti/tw)|0,px=ti-py*tw;const pv=prov[ti];
  ...
  else if(rcc===cc&&pv>=0&&prov[rti]>=0&&prov[rti]!==pv){if(((py)&1)===0)setpx((px+1)*SC-1,py*SC,pcol);}
  if(py<th-1){const dti=ti+tw,dcc=claim[dti];
    ...
    else if(dcc===cc&&pv>=0&&prov[dti]>=0&&prov[dti]!==pv){if(((px)&1)===0)setpx(px*SC,(py+1)*SC-1,pcol);}}}
const out=`/tmp/prov_${GAP>0?"on":"off"}_${STEP}.png`;
