/* V2 M1 PORT
 * source: src/sim/moistureSolver.js; deviations: Math transcendental calls use v2 dmath; local imports point at the copied v2 supplier.
...
export function solveMoisture(W, H, elevation, windX, windY, temperature, params = {}) {
  ...
  for (let my = 0; my < mH; my++) for (let mx = 0; mx < mW; mx++) {
    ...
    if (hasTemp) {
      ...
      temp[mi] = Math.max(0, Math.min(1,
        0.92 - dpow(lat, 1.5) * 0.50 - dpow(lat, 6) * 0.80
        + dexp(-((lat - 0.20) * (lat - 0.20)) / (2 * 0.08 * 0.08)) * 0.06
        - e * (0.65 + 0.8 * lat)));
    }
  }
  ...
  const diffPerCell = dpow(_moistDiffuse, (360 / mW) / _DIFF_REF_DEG);
  ...
  for (let step = 0; step < STEPS; step++) {
    ...
    for (let my = 1; my < mH - 1; my++) for (let mx = 0; mx < mW; mx++) {
      ...
      const coreSubsidence = dexp(-(subtropDist * subtropDist) / (2 * 6 * 6));
      const wideSubsidence = dexp(-(subtropDist * subtropDist) / (2 * 14 * 14));
      ...
      if (temp[ci] > 0.45 && moist > 0.05) {
        ...
        const itczMonsoon = dexp(-((latSgn - _itczLat) * (latSgn - _itczLat)) / (2 * 8 * 8));
        ...
        const itczEquator = dexp(-(latSgn * latSgn) / (2 * 9 * 9)) * 0.9
        ...
        const midlatFactor = dexp(-((latDeg - 45) * (latDeg - 45)) / (2 * 12 * 12)) * 0.3;
      ...
      if (_itczLat !== 0 && latSgn * _itczLat < 0) {              // this hemisphere is in winter
        ...
        const front = dexp(-((aLat - _frontLat) * (aLat - _frontLat)) / (2 * _frontWidth * _frontWidth));
        if (front > 0.03) {
          moist += front * _frontStr * dexp(-distToOcean[ci] / (_frontReach * mW / 240));
  ...
  for (let i = 0; i < mN; i++) {
    if (isOcean[i]) {
      ...
      const tCap = Math.min(1, dpow(Math.max(0, temp[i]), 0.7) * 1.1 + 0.02);
