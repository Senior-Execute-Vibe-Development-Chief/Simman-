/* V2 M1 PORT
 * source: src/sim/resourceGen.js; deviations: local classifier import points at the copied v2 supplier; mine scatter radii are converted from tiles to km.
 * source commit: 97f51dd7c3a3142bfbb366f2e08491f582367e30
 */
import { EARTH_SURFACE_KM2, MINE_SCATTER_RADIUS_KM, MINE_SCATTER_SMALL_RADIUS_KM } from "../../sim/constants.ts";
...
export function generateResources(tw, th, tElev, tTemp, tMoist, tCoast, world, seed, rivers, tDryFrac, tSummerDry) {
  ...
  function scatterMines(resourceId, density, radiusKm, peakRichness, candidateTest, scoreFn) {
    const radius = Math.max(1, Math.ceil(radiusKm / kmPerTile));
  ...
  scatterMines('copper', 0.005, MINE_SCATTER_RADIUS_KM, 0.9,
    (ti) => tElev[ti] > 0.02,
    (ti) => {
  ...
  scatterMines('tin', 0.002, MINE_SCATTER_SMALL_RADIUS_KM, 0.85,
    (ti) => tElev[ti] > 0.02,
    (ti) => {
      let s = 0.4;
      if (tElev[ti] > 0.10) s += 0.15;
  ...
  scatterMines('precious', 0.0015, MINE_SCATTER_RADIUS_KM, 0.95,
    (ti) => tElev[ti] > 0.02,
    (ti) => {
      let s = 0.4;
  ...
  scatterMines('gems', 0.001, MINE_SCATTER_SMALL_RADIUS_KM, 0.8,
    (ti) => tElev[ti] > 0.03,
    (ti) => {
      let s = 0.3;
