/* V2 M1 PORT
 * source: src/sim/tectonicGen.js; deviations: Math transcendental calls use v2 dmath; local imports point at the copied v2 supplier.
...
for (let i = 0; i < numPlates; i++) {
  ...
  const nucAngle = rng() * PI * 2;
  const nucOffset = 0.02 + rng() * 0.08;
  const nucX = cx + dcos(nucAngle) * nucOffset;
  const nucY = cy + dsin(nucAngle) * nucOffset;
...
for (let py = 0; py < ppH; py++) for (let px = 0; px < ppW; px++) {
  ...
  const plateLat = (ny - 0.5) * PI;
  const plateCosLat = Math.max(0.15, dcos(plateLat));
...
for (let pi = 0; pi < numPlates; pi++) {
  ...
  for (let s = 0; s < numSubs; s++) {
    ...
    if (s === 0) { sx = cx; sy = cy; }
      ...
      let ang;
      if (rng() < pull * 0.85) ang = datan2((cy - parent.y) / AR, cx - parent.x) + (rng() - 0.5) * 2.0;
      else ang = (rng() < elong) ? growAngle + (rng() - 0.5) * 1.7 : rng() * PI * 2;
      const step = (parent.r + baseR) * (0.5 + rng() * 0.32);
      sx = parent.x + dcos(ang) * step;
      sy = parent.y + dsin(ang) * step * AR;
    }
    placed.push({ x: sx, y: sy, r: baseR });
    const aspect = s === 0 ? 1 + rng() * 0.5 : 1.2 + rng() * rng() * 2.2;
    const rot = rng() < 0.55 ? growAngle + (rng() - 0.5) * 0.9 : rng() * PI;
    posStamps.push({
      cx: sx, cy: sy,
      rx: baseR * aspect, ry: baseR / aspect * AR,
      rot, cos: dcos(rot), sin: dsin(rot),
  ...
  for (let n = 0; n < numNegs; n++) {
    ...
    const rot = rng() * PI;
    negStamps.push({
      cx: anchor.x + dcos(ang) * dist,
      cy: anchor.y + dsin(ang) * dist,
...
for (let ey = 0; ey < ewH; ey++) for (let ex = 0; ex < ewW; ex++) {
  ...
  const lat = (ny - 0.5) * PI; // -π/2 to π/2
  const cosLat = Math.max(0.15, dcos(lat)); // clamp to prevent infinity at poles
  for (const c of posStamps) {
...
const mtnBroad = new Float32Array(N);
{
  ...
  for (let k = -radius; k <= radius; k++) {
    const v = dexp(-k * k / (2 * sigma * sigma));
...
if (p('erodeDropsPerPixel', 1.5) > 0) {
  ...
  for (let drop = 0; drop < dropCount; drop++) {
    ...
    for (let step = 0; step < maxLife; step++) {
      ...
      if (len < 1e-6) { const a = rng() * 6.283; ddx = dcos(a); ddy = dsin(a); }
...
for (let my = 0; my < mH; my++) for (let mx = 0; mx < mW; mx++) {
  ...
  tGrid[my * mW + mx] = Math.max(0, Math.min(1, 0.92 - dpow(tLat2, 1.5) * 0.50 - dpow(tLat2, 6) * 0.80
    + dexp(-((tLat2 - 0.20) * (tLat2 - 0.20)) / (2 * 0.08 * 0.08)) * 0.06 - Math.max(0, e2) * (0.65 + 0.8 * tLat2)));
}
...
for (let step = 0; step < 25; step++) {
  ...
  for (let my = 1; my < mH - 1; my++) for (let mx = 0; mx < mW; mx++) {
    ...
    const localT = Math.max(0, Math.min(1, 0.92 - dpow(tLat2, 1.5) * 0.50 - dpow(tLat2, 6) * 0.80
      + dexp(-((tLat2 - 0.20) * (tLat2 - 0.20)) / (2 * 0.08 * 0.08)) * 0.06 - Math.max(0, e2) * (0.65 + 0.8 * tLat2)));
    if (e2 <= 0) {
...
for (let y = 0; y < H; y++) for (let x = 0; x < W; x++) {
  ...
  const subtropHeat = dexp(-((tLat - 0.20) * (tLat - 0.20)) / (2 * 0.08 * 0.08)) * 0.06;
  ...
  const baseTemp = 0.92 - dpow(tLat, 1.5) * 0.50 - dpow(tLat, 6) * 0.80 + subtropHeat - Math.max(0, e) * (0.65 + 0.8 * tLat) + sg(nfTemp, x, y) * 0.08
