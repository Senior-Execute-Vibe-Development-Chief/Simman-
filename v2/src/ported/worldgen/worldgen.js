/* V2 M1 PORT
 * source: src/sim/worldgen.js; deviations: Math transcendental calls use v2 dmath; local imports point at the copied v2 supplier; v1's EARTH_STRAITS carve is gone — no cell is opened, the channels the raster cannot hold are carried on the edges between cells by the baked crossing table (W22; the oracle removes the carve from its v1 copy so the two elevations still agree byte for byte).
 * source commit: 97f51dd7c3a3142bfbb366f2e08491f582367e30
 */
import { dexp, dpow, dcos, dsin, datan2 } from "../../sim/dmath.ts";
import { MATH_PI as PI } from "../../sim/constants.ts";
...
let landShape=null;let passClimb=null;
if(preset==="earth"){
// ── Earth mode: use real heightmap data ──
const eData=decodeEarth(EARTH_ELEV);
const fData=decodeLandFrac(LAND_FRAC,eData);landFraction=new Float32Array(W*H);landShape=decodeLandShape(LAND_SHAPE);passClimb=decodePassClimb(W,H);crossings=decodeCrossings(W,H);
...
temperature[i]=Math.max(0,Math.min(1,0.85-0.66*lat*lat+0.18*lat*lat*lat*lat+dexp(-((lat-0.12)*(lat-0.12))/(2*0.14*0.14))*0.030-Math.max(0,elevation[i])*(.48+.12*lat)-Math.max(0,(ny-0.5)*2-0.53)*0.85+fbm(nx*3+80,ny*3+80,3,2,.5)*.03));}
...
const subtropDry=dexp(-((lat-.28)*(lat-.28))/(2*.08*.08))*.35*(1-coastProx*.5);// subtropical HP, softened + widened for realism
const tempWet=dexp(-((lat-.55)*(lat-.55))/.025)*.22;// temperate westerlies
...
// Nothing is carved (W22): a channel the raster cannot hold as a cell lies on the edge between two cells, in `crossings`, and the ocean fill reads it there.
// Coast distance BFS
const CDT=4,CDW=Math.ceil(W/CDT),CDH=Math.ceil(H/CDT);
...
const subtropW=dexp(-((aLat-30)*(aLat-30))/(2*9*9));  // warm western boundary, ~21-39°
const subpolarW=dexp(-((aLat-54)*(aLat-54))/(2*9*9)); // cold western boundary, ~45-63°
const subtropE=dexp(-((aLat-23)*(aLat-23))/(2*13*13));// cold eastern boundary,  ~10-36°
const driftE=dexp(-((aLat-53)*(aLat-53))/(2*11*11));  // warm eastern drift,     ~42-64°
...
const oceanKeep=dpow(0.96,360/W),landDecay=dpow(0.94,360/W);// ~4%/° over sea, ~6%/° over land
...
tGrid[my*mW2+mx]=Math.max(0,Math.min(1,0.85-0.66*lt*lt+0.18*lt*lt*lt*lt+dexp(-((lt-0.15)*(lt-0.15))/(2*0.10*0.10))*0.035-Math.max(0,e2)*(.48+.12*lt)-shCool));}
...
const locT=Math.max(0,Math.min(1,0.85-0.66*lt*lt+0.18*lt*lt*lt*lt+dexp(-((lt-0.15)*(lt-0.15))/(2*0.10*0.10))*0.035-Math.max(0,e2)*(.48+.12*lt)-Math.max(0,(py/H-0.5)*2-0.53)*0.85));
...
const shE=dexp(-((tLat-0.12)*(tLat-0.12))/(2*0.14*0.14))*0.030;// equatorial+subtropical lift (humid tropics sit ~27°C); fades out by ~40° so it doesn't warm mid-latitudes
...
const beltLat=dexp(-((tLat-0.275)*(tLat-0.275))/(2*0.095*0.095));// subtropical-high belt, widened to ~16-36° so it also reaches the poleward deserts (US SW, N Iran, Patagonia) now that the monsoon spare protects the wet subtropics
...
const llj=dexp(-((latS-0.30)*(latS-0.30))/(2*0.11*0.11))    // ~15-39°S (Chaco→Pampas)
  *dexp(-((lonDeg+60)*(lonDeg+60))/(2*7*7))                 // lee of the Andes, ~53-67°W
...
const araLon=dexp(-((lonDeg-46)*(lonDeg-46))/(2*9*9));
const araLat=dexp(-((tLat-0.235)*(tLat-0.235))/(2*0.088*0.088));
...
const patShadow=e>0?dexp(-((latS-0.49)*(latS-0.49))/(2*0.095*0.095))*dexp(-((lonDeg+66)*(lonDeg+66))/(2*3.2*3.2))*Math.max(0,1-Math.max(0,e-0.05)*4)*0.62:0;
// ── Combine the drying ──
...
const foreland=dexp(-((latS+0.244)*(latS+0.244))/(2*0.078*0.078))  // ~22°N (latS<0 is N)
  *dexp(-((lonDeg-91)*(lonDeg-91))/(2*8*8))                        // Ganges→Bengal→Irrawaddy
...
const asiaLon=dexp(-((lonDeg-86)*(lonDeg-86))/(2*15*15));
const asiaLat=dexp(-((tLat-0.42)*(tLat-0.42))/(2*0.10*0.10));
mo=Math.min(1,mo+asiaLon*asiaLat*0.12);
const dry=Math.max(0,1-mo/0.35);// 1 = bone-dry, 0 = humid
const desertHeat=dry*0.09*dexp(-((tLat-0.22)*(tLat-0.22))/(2*0.13*0.13));// peaks on the 13-30° HOT-DESERT belt (Sahel, Sahara, Arabia — Earth's hottest annual means), not the 33° subtropics
...
const interiorCold=Math.min(0.06,conti*dpow(Math.max(0,tLat-0.40),2)*1.8);
...
+dpow(Math.max(0,fbm(nx*3+20,ny*3+20,4,2.2,.5)),2)*.4// mountain ranges
+fbm(nx*14+7,ny*14+7,3,2,.4)*.06// fine detail
+dpow(1-Math.abs(fbm(nx*2.5+40,ny*2.5+40,3,2.1,.5)),4)*.25;// ridges
...
e-=dpow(Math.max(0,fbm(nx*4+60,ny*4+60,3,2,.5)+.1),2)*.15;
elevation[i]=Math.max(0.005,e);
// Moisture: climate zones + elevation effects
const tropWet=Math.max(0,1-lat*2.5);
const subtropDry=dexp(-((lat-.28)*(lat-.28))/(2*.08*.08))*.30;// subtropical HP belt, softened
const tempWet=dexp(-((lat-.55)*(lat-.55))/.025)*.20;
const polarDry=Math.max(0,(lat-.75))*.25;
...
moisture[i]=Math.max(.02,Math.min(1,m));
temperature[i]=Math.max(0,Math.min(1,0.92-dpow(lat,1.5)*0.50-dpow(lat,6)*0.80-Math.max(0,e)*(.4+.8*lat)+fbm(nx*3+80,ny*3+80,3,2,.5)*.1));}
...
subs.push({cx:cx+dcos(ang)*dist,cy:cy+dsin(ang)*dist,
rx:baseR*aspect,ry:baseR/aspect,rot:rng()*PI,str:s===0?.8+rng()*.4:.5+rng()*.4,no:no+s*17});}
// 0-2 negative stamps carve bays/gulfs
const negs=[];const numNegs=Math.floor(rng()*2.5);
for(let n=0;n<numNegs;n++){
const ang=rng()*PI*2,dist=.02+rng()*.06;
negs.push({cx:cx+dcos(ang)*dist,cy:cy+dsin(ang)*dist,
rx:.02+rng()*.04,ry:.015+rng()*.03,rot:rng()*PI,str:.25+rng()*.3,no:no+50+n*13});}
...
for(const cont of continents){for(const c of cont.subs){c.cos=dcos(c.rot);c.sin=dsin(c.rot);posStamps.push(c);}
for(const c of cont.negs){c.cos=dcos(c.rot);c.sin=dsin(c.rot);negStamps.push(c);}}
...
let dd=Math.sqrt(dpow((dx*c.cos+dy*c.sin)/c.rx,2)+dpow((-dx*c.sin+dy*c.cos)/c.ry,2));
...
e=dpow(Math.max(0,e),0.85)*1.2;e=Math.max(0.003,e);}
...
elevation[i]=e;temperature[i]=Math.max(0,Math.min(1,0.92-dpow(lat,1.5)*0.50-dpow(lat,6)*0.80-Math.max(0,e)*(.4+.8*lat)+fbm(nx*3+80,ny*3+80,3,2,.5)*.1));}
...
const tropWet=Math.max(0,1-lat*2.5);
const subtropDry=dexp(-((lat-.28)*(lat-.28))/(2*.08*.08))*.35*(1-coastProx*.5);
const tempWet=dexp(-((lat-.55)*(lat-.55))/.025)*.22;
...
const latSwing=26*dpow(lat,1.35);                       // °C, continental-limit swing
...
return{elevation,moisture,temperature,dryFrac,summerDry,tAmp,warmRainFrac,coastal,swamp,width:W,height:H,preset,pixPlate:tecPlates,windX:tecWindX||null,windY:tecWindY||null,crossings,landFraction,landShape,landShapeWidth:LAND_SHAPE_W,landShapeHeight:LAND_SHAPE_H,passClimb,_seed:seed};}
