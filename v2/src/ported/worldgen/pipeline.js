...
import { mkRng, hash32 } from "../rng.ts";
import {
  ANCESTRY_HOP_FREE_KM,
  ANCESTRY_OCEAN_EFOLD_KM,
  EARTH_SURFACE_KM2,
  LAKE_MOISTURE_RADIUS_KM,
  VOLCANIC_FULL_KM,
  VOLCANIC_INFLUENCE_KM,
...
const tFactor=Math.min(1,Math.max(0,(t-0.57)/0.13))*Math.min(1,1-dpow(Math.max(0,t-0.88),2)*1.5);
const mFactor=dexp(-((m-0.45)*(m-0.45))/(2*0.22*0.22));
...
function generateAncestry(tw, th, tElev, tTemp, tMoist, tDiff, tFert, seed, preset) {
  const N = tw * th;
  const kmPerTile = Math.sqrt(EARTH_SURFACE_KM2 / N);
  const hopFreeTiles = Math.max(1, Math.ceil(ANCESTRY_HOP_FREE_KM / kmPerTile));
  ...
  const stepCost = (from, to, diag) => (
    tElev[to] <= 0 ? ANC_OCEAN_STEP * dpow(ANC_OCEAN_DEEP, Math.max(0, (distLand[to] - hopFreeTiles) * kmPerTile / ANCESTRY_OCEAN_EFOLD_KM)) * diag
  ...
  if (origin < 0) {
    ...
    for (let ti = 0; ti < N; ti++) {
      ...
      const warm = dexp(-((tTemp[ti] - 0.74) * (tTemp[ti] - 0.74)) / (2 * 0.12 * 0.12));   // deep-tropical optimum
  ...
  for (const ti of land) {
    ...
    const fdens = Math.min(1, dpow(cap[ti], 1.3) * (1 + 0.5 * Math.min(1, tDiff[ti])) * (1 - ANC_FRONTIER_THIN * arrN[ti] * arrN[ti]));
  ...
  for (const ti of land) {
    ...
    const dens = Math.min(1, dpow(cap[ti], 1.7) * (1 + ANC_ISO_W * Math.min(1, tDiff[ti])) * dpow(1 - arrN[ti], ANC_RESIDENCE_P));
...
const t2=Math.min(1,effDist/effHW);const falloff=0.5+0.5*dcos(t2*PI);
...
if(rivers.lake){
const lakeRadius=Math.max(1,Math.ceil(LAKE_MOISTURE_RADIUS_KM/Math.sqrt(rivers.km2PerTile||1)));const lakeMoistPeak=0.20;
for(let ti=0;ti<tw*th;ti++){
...
const falloff=0.5+0.5*dcos(effDist/lakeRadius*PI);
...
const tempFit=dexp(-((t-0.45)*(t-0.45))/(2*0.10*0.10));// peak at t=0.45
// Semi-arid to moderate moisture: grassland/steppe zone (not forest, not desert)
const moistFit=dexp(-((m-0.28)*(m-0.28))/(2*0.10*0.10));// peak at m=0.28
...
const kmPerTile=Math.sqrt(rivers.km2PerTile||1);
bDist=new Float32Array(tw*th);bDist.fill(Infinity);
const bdQ=[];
...
if(cd>=VOLCANIC_INFLUENCE_KM)continue;// physical influence radius
for(const[dx,dy]of DIRS){const nx=(cx+dx+tw)%tw,ny=cy+dy;if(ny<0||ny>=th)continue;
const ni=ny*tw+nx;if(bDist[ni]<=cd+kmPerTile||tElev[ni]<=0)continue;
bDist[ni]=cd+kmPerTile;bdQ.push(ni);}}
// Apply volcanic bonus: strongest at boundary, decays with distance
for(let ti=0;ti<tw*th;ti++){if(bDist[ti]>=VOLCANIC_FULL_KM||tElev[ti]<=0)continue;
// Only apply where there's enough moisture for agriculture
if(tMoist[ti]<0.15)continue;
const proximity=1-bDist[ti]/VOLCANIC_FULL_KM;// 1.0 at boundary, 0 at full influence
