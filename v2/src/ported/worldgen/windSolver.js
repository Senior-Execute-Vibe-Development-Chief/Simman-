/* V2 M1 PORT
 * source: src/sim/windSolver.js; deviations: Math transcendental calls use v2 dmath; local imports point at the copied v2 supplier.
 * source commit: 97f51dd7c3a3142bfbb366f2e08491f582367e30
 */
import { dexp, dcos, dsin } from "../../sim/dmath.ts";
import { MATH_PI as V2_PI } from "../../sim/constants.ts";
// ══════════════════════════════════════════════════════════════════
...
export function solveWind(W, H, elevation, fbm, params = {}, noiseSeed = 42) {
  ...
  for (let wy = 0; wy < wH; wy++) {
    ...
    for (let wx = 0; wx < wW; wx++) {
      ...
      const subtropFactor = dexp(-((absLat * 90 - 25) * (absLat * 90 - 25)) / 600);
  ...
  for (let wy = 0; wy < wH; wy++) {
    ...
    const itcz = -0.5 * dexp(-((latDegSigned - itczLat) * (latDegSigned - itczLat)) / 250);

    // Subtropical highs: these are the KEY feature for anticyclonic gyres
    // They should be strong and broad
    const subtropN = 0.8 * dexp(-((latDegSigned - 30) * (latDegSigned - 30)) / 200);
    const subtropS = 0.8 * dexp(-((latDegSigned + 30) * (latDegSigned + 30)) / 200);

    // Subpolar lows (Icelandic Low, Aleutian Low)
    const subpolarN = -0.5 * dexp(-((latDegSigned - 60) * (latDegSigned - 60)) / 180);
    const subpolarS = -0.5 * dexp(-((latDegSigned + 55) * (latDegSigned + 55)) / 180);

    // Polar highs
    const polarN = 0.2 * dexp(-((latDegSigned - 85) * (latDegSigned - 85)) / 100);
    const polarS = 0.3 * dexp(-((latDegSigned + 85) * (latDegSigned + 85)) / 100);
    ...
    for (let wx = 0; wx < wW; wx++) {
      ...
      const subtropWeight = dexp(-((absLat * 90 - 30) * (absLat * 90 - 30)) / 250);
      ...
      const stormBelt = dexp(-((absLat * 90 - 45) * (absLat * 90 - 45)) / 400);
  ...
  for (let wy = 1; wy < wH - 1; wy++) {
    ...
    const f = -dsin(latSigned * PI / 2) * _coriolisStr;
    const cosLat = dcos(Math.abs(latSigned) * PI / 2);
  ...
  for (let wy = 1; wy < wH - 1; wy++) {
    ...
    const hadleyMerid = -Math.sign(latSigned) *
      dexp(-((absLat * 90 - 15) * (absLat * 90 - 15)) / 350) * 0.25 * _hadleyStr;

    // Ferrel: poleward, 30-60° (weaker)
    const ferrelMerid = Math.sign(latSigned) *
      dexp(-((absLat * 90 - 45) * (absLat * 90 - 45)) / 300) * 0.1 * _hadleyStr;
  ...
  for (let wy = 1; wy < wH - 1; wy++) {
    ...
    const latFactor = 0.5 + 0.5 * dexp(-((absLat * 90 - 45) * (absLat * 90 - 45)) / 500);
