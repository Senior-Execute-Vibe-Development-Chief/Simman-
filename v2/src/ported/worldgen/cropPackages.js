/* V2 M1 PORT
 * source: src/sim/cropPackages.js; deviations: dexp uses v2 dexp; v1 tuning import is replaced by its reviewed M1 values.
...
export const CROP_PACKAGES = [
  ...
  // tTolEarly (1): PREHISTORIC maize was PHOTOPERIOD-SENSITIVE —
  ...
  // rice's proto-domestication tail). Gated by 1 (v7 save-regime
...
export function pkgClimateBell(pkg, t, m) {
  // 1: use a package's PREHISTORIC temperature tolerance where
  ...
  // temperature bell.
  const tTol = (1 && pkg.tTolEarly) ? pkg.tTolEarly : pkg.tTol;
