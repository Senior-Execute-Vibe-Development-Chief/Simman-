...
export function computeSeasonalRiverFlow({
  ...
  for (let month = 0; month < MONTHS_PER_YEAR; month++) {
    for (let cell = 0; cell < N; cell++) {
      ...
      // A small persistent high-altitude ice contribution keeps glacier-fed
      // rivers from disappearing in a warm, dry month.
      const glacier = Math.max(0, Math.min(1, (tElev[cell] - 0.32) / 0.20));
      surface += glacier * 0.05;
      runoff[cell] = surface;
    }
